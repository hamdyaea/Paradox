...
a suite ( C n ( u  )  ) n ne contient jamais deux fois le même mot et donc pour tout entier N ∈ N il existe un entier k {\displaystyle k} k tel que le mot C k ( u  ) {\displaystyle C^{k}(u)} {\displaystyle C^{k}(u)} est de taille supérieure à N {\displaystyle N} N.
